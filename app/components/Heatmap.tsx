...
import { eachDayOfInterval, endOfYear, format, getDay, isSameDay, startOfYear, subMonths } from 'date-fns';
...
export function Heatmap({ data, color }: HeatmapProps) {
  ...
  return (
    <div className="w-full overflow-x-auto pb-4">
            ...
            <div key={weekIndex} className="flex flex-col gap-1">
              ...
              <div className="h-3 text-xs text-gray-400 mb-1">
                {week.some(d => d.getTime() !== 0 && d.getDate() <= 7) && 
                 ...
                 format(week.find(d => d.getTime() !== 0 && d.getDate() <= 7)!, 'MMM')}
              </div>
              ...
              {week.map((day, dayIndex) => {
                ...
                return (
                  <div
                    ...
                    data-tooltip-content={`${count} logs on ${format(day, 'MMM do, yyyy')}`}
                  />
                );
              })}
            </div>
      ...
      <ReactTooltip id="heatmap-tooltip" className="z-50 !rounded-lg !text-xs !px-3 !py-2" />
    </div>
  );
}
